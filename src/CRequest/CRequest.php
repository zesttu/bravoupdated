...
class CRequest {
	...
	//Constructor = decide which type of url should be generated as outgoing links.
  ...
  public function Init($baseUrl = null) {
    ...
    //Kolla om det är en querystring länk
  ...
  $baseUrl = !empty($baseUrl) ? $baseUrl : "{$parts['scheme']}://{$parts['host']}". (isset($parts['port']) ? ":{$parts['port']}" : '') . rtrim(dirname($scriptName), '/');
    ...
    $this->arguments    = $arguments;
  	...
  	$url .= $_SERVER["SERVER_PORT"] . $serverPort . htmlspecialchars($_SERVER["REQUEST_URI"]);
  	return $url;
  	}
